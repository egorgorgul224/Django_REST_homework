...
```
Ссылка для контроллера: адрес/user/<int:pk>/payments/
```
...
2. PaymentSerializer - сериализатор для модели Payment. В Meta класс предоставлен доступ ко всем полям. Для
